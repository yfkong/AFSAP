...
#algorithm info
print "An algorithm framework for the service area problem (AFSAP) v1.01. " 
print "by Dr. Yunfeng Kong (yfkong@henu.edu.cn), Henan University, China."
print "March 31, 2019."

import sys,os,random,time,copy,math

#mip solver
mip_solvers=['cplex','cbc'] #MIP solvers supported 
mip_solver='cplex'  #MIP solver "cplex", "cbc" or ""
try:
    import cplex
    print "CPLEX Python API: available!"
except: 
    print "CPLEX Python API: unavailable!"
try:
    import pulp
    s=pulp.solvers.COIN_CMD()
    if s.available()==False:
        mip_solvers.remove('cplex')
        print "PuLP CPLEX：unavailable!"
    else: print "PulP CPLEX: available!"		
    s=pulp.solvers.CPLEX_CMD()
    if s.available()==False:
        mip_solvers.remove('cbc')
        print "PuLP CBC: unavailable!"
    else: print "PuLP CBC: available!"		
except: 
    print "PulP: unavailable!"

if len(mip_solvers)==0: 
    mip_solver=''
else:
    mip_solver=mip_solvers[0]
...
maxloops = 100 # maximum number of search loops for GA
...
def ga(numf,m,timelimit,mthd,crate, mrate,spp,myseed):
    ...
    if m<2: 
        print "the number of facilities must be greater than 1."	
        return
    cur_crate=crate
    cur_mrate=mrate
    heuristic_time_limit=timelimit
    ...
    while 1:
        ...
        print "cur", int(biobjective*100)/100.0,int(objective_overload),"best",int(best_biobjective_global*100)/100.0, 
...
#        if is_spp_modeling==1 and loop%spp_loops==spp_loops-1:
        ...
        print [b1,b2,len(population)], not_improved,int((time.time()-t_ga)*100)/100.0
