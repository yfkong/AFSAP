...
import sys,os,random,time,copy,math,pulp
#instance info
instancefile=""
nodes=[]
num_units=-1
...
num_districts=-1 # defined by school group info
max_num_facility=999
pop_dis_coeff=10000.0
NFlist=[]
...
is_spp_modeling=1
spp_loops=50
#is_params_adjusting=1
mip_solvers=['cplex','cbc']
mip_solver='cplex'  #cplex or cbc
...
#record region rid in current solution
...
def is_edge_unit(uid):
    global time_check_edge_niut
    if uid in centersID:
        return False
    t=time.time()
    rlist = [node_groups[x] for x in node_neighbors[uid]]
    #rlist.sort()
    time_check_edge_niut+=time.time()-t
    #if rlist[0]==rlist[-1]:
    if len(set(rlist))>1:
        return False
    return True
...
def update_district_info():
    ...
    for k in range(num_districts):
        ...
        if len(ulist)<=0:
            print "region has no any unit", k
            continue
        ck=centersID[k]
        ...
        if district_info[k][4]<0:
            district_info[k][4] =0
        #if district_info[k][3] != capacities[k]:
        #    print "capacities error in ", k,district_info[k][3], capacities[k]
...
# def find_frag_unit2():
    ...
    # for j in range(num_districts):
        # ulist1=[centersID[j]]
        # ulist2=[x for x in range(num_units) if node_groups[x]==j and x!=centersID[j]]
        # while 1:
            # found=0
            # for x in ulist1:
                # for y in ulist2:
                    # if y in node_neighbors[x]:
                        # ulist1.append(y)
                        # ulist2.remove(y)
                        # found=1
            # if found==0:
...
def repare_current_solution():
    ...
    for x in frag_units:
        node_groups[x]=-1
    repare_overlay_solution()
    #print "rep",node_groups
...
def repare_overlay_solution():
    ...
    while 1:
        ...
        for i in range(len(neighbors)):
            ...
            gap= district_info[rid][1]+ nodes[nid][3] - facilityCapacity[rid]
            #if gap<=0: gap=0
...
def update_best_solution():
    ...
    if check_solution_continuality_feasibility(node_groups)==0:
        print "check_solution_continuality_feasibility!!!"
        return improve
    biobj=biobjective
...
def isbetter(obj_old,obj_new,bal_old,bal_new):
    ...
    if acceptanceRule=="rrt":
        if biobj_new < best_biobjective*(1+devition/num_units):
            dev_move_counter+=1
            return 1
    return 0
...
def one_unit_move():
    ...
    for n_id in ulist:
        ...
        for new_r_id in neighbor_regions(n_id):
            ...
            sol=node_groups[:]
            sol[n_id] = new_r_id
            if check_continuality_feasibility(sol,r_id)==0:
                break
            count_op0+=1
    ...
    for n_id1 in ulist:
        ...
        for n_id2 in ulist:
            ...
            for new_r_id2 in neighbor_regions(n_id2):
                ...
                sol[n_id1] = new_r_id1
                sol[n_id2] = new_r_id2
                if check_continuality_feasibility(sol,r_id1)==0:
                    movelist.append(n_id1)
                    break
                    
                if check_continuality_feasibility(sol,r_id2)==0:
                    movelist.append(n_id2)
                    break
                ...
                find_best_solution += update_best_solution()
                success_move=1
                #print n_id1,r_id1,new_r_id1,n_id2,r_id2,new_r_id2,obj-biobjective
                break
...
#                    continue
                ...
                for new_r_id3 in neighbor_regions(n_id3):
                    ...
                    if check_continuality_feasibility(sol,r_id1)==0:
                        movelist.append(n_id1)
                        break
                    if r_id1!=r_id2 and check_continuality_feasibility(sol,r_id2)==0:
                        movelist.append(n_id2)
                        break
                    if r_id1!=r_id3 and r_id2!=r_id3 and check_continuality_feasibility(sol,r_id3)==0:
                        movelist.append(n_id3)
                        break
                    count_op4+=1
                    node_groups[n_id1] = new_r_id1
                    ...
                    obj=biobjective
                    update_district_info()
                    #print n_id1,r_id1,new_r_id1,n_id2,r_id2,new_r_id2,n_id3,r_id3,new_r_id3,obj-biobjective
                    improve = 1
                    find_best_solution +=  update_best_solution()
                    break
                if improve == 1:
...
# local search with or without threshhold (devition>0 or devition=0)
def RRT_local_search():
    ...
    for op in operators:
        ...
        if op == 2:
            three_unit_move()
            #update_region_pool_all()
        #print "operator", op, "move,obj=", objective, objective_overload,
        #print " bestobj=", best_objective, best_objective_overload,improved
    return
...
def vnd_op_search(op):
    ...
    if op == 2:
        three_unit_move()
    #print "bestg", best_biobjective_global, "op", op, "obj=", biobjective, objective, objective_overload,improved
    return
...
def readfile(f1,f2):
    ...
    facilityCapacity=[]
    facilityCost=[]
    #print "centers:",centersID
    #used in initial solution
    #avg_dis=sum([nodedij[i][node_groups[i]]*nodes[i][3] for i in range(num_units)])/total_pop
    ...
    print "the total and averaged nearst distance to facility",dis,avg_dis_min
    print "demand and supply",total_pop,sum(facilityCapacity)
...
##            prob +=s == 1
    ...
    #print node_groups
    #print sol
##    a=[nodes[x][6] for x in centersID]
##    print a
##    a=[nodes[x][7] for x in centersID]
##    print a
    return pulp.value(prob.objective)

#build and solve a mip model (DPMILP)    
def mipmodel():
    try:
        import cplex
    except:
        print "cplex is not supported"
        #return
...
##            idx+=1
    ...
    alpha_coeff=avg_dis_min*pop_dis_coeff    
    mip_file="_MIP_PDP2_"+str(num_districts)+".lp"
    f = open(mip_file,"w")
    f.write("Minimize\nobj: ")
    for k in range(num_districts):
...
##            f.write(" w_" +str(i)+ "_"+ str(k)+"\n")
    ...
    #c.populate_solution_pool()
    print "cplex status and obj:",c.solution.get_status_string(),c.solution.get_objective_value()
    ...
    print "model solution",biobjective,objective,objective_overload,time.time()-tstart
...
def set_solver_params(numf,solver,start,timelimit,spp,s):
    ...
    if numf != len(centersID):
        print "invalid number of facilities"
        return
...
def print_solution():
    print "1 the objective,distance and balence", biobjective,objective,objective_overload
    print "2 the centers:",
    for x in centersID:
        print nodes[x][4],
    print
    print "3 the solution:"
    print "nodeID,x,y,demand,capacity,districtID"
    for i in range(num_units):
        print nodes[i][4],nodes[i][1],nodes[i][2],nodes[i][3],nodes[i][5],node_groups[i]
    print "4 the disticts:"
    print "DistrictId, FID, No.units, pop, cap, dis"
    for i in range(num_districts):
        print i,centersID[i],district_info[i][0],district_info[i][1],district_info[i][3], int(district_info[i][2]*100)/100
            
def print_current_solution():
    print "centers:",centersID
    print "districts", node_groups
    print "objective,distance and pop balence", biobjective,objective,objective_overload
    print "districts: Idx, No.units, pop, dis"
    for i in range(num_districts):
        print i, district_info[i][0],district_info[i][1], district_info[i][2]
...
def solve():
    ...
    #update_district_info()
    #print "obj and facilities",obj,len(centersID),centersID
    ...
    best_centersID=[]
    move_count=0
    devition=intial_devition
    last_devition=devition
    rule=acceptanceRule
    t_begin=time.time()
    t_h=time.time()
    ...
    if acceptanceRule in ["ils","vns","ilsvnd"]: #pop-based local search
        ...
        for multi_start in range(multi_start_count):
            ...
            multi_start+=1
            t2=time.time()
            initial_solution(myseed+multi_start)
            #init_sol_model()
            feasible=check_solution_continuality_feasibility(node_groups)
            if feasible==0:
                print "check_solution_continuality_feasibility(node_groups)..."
                print node_groups
                return
            update_district_info()
            rule=acceptanceRule
            ...
            print "initial solution", multi_start,":",int(biobjective),int(objective), int(objective_overload)
    ...
    if acceptanceRule=="mip":
        ...
        update_best_solution()
        RRT_local_search()
        print "MIP results:",best_biobjective,best_objective,best_objective_overload
        is_spp_modeling=0
    if acceptanceRule=="vns":
        ...
        while 1:
            ...
            print "pop:",[int(solutions[0][0]),int(solutions[min(multi_start_count,len(solutions))-1][0])],
            print acceptanceRule,loop,"bestg",int(best_biobjective_global*100)/100.0,int(best_overload_global),
            #print "cbest", int(best_biobjective),int(best_objective),int(best_objective_overload),
            print "current", int(biobjective*100)/100.0,int(objective_overload),int((time.time()-t_h)*100)/100.0,not_improve_best
            ...
            if not_improve_best%spp_loops==spp_loops-1:
                print "^",
    ...
    if acceptanceRule=="ilsvnd" or acceptanceRule=="ils":
        ...
        while 1:
            loop+=1
            print acceptanceRule,"pop:",[int(solutions[0][0]),int(solutions[min(multi_start_count,len(solutions))-1][0])],
            move_count=0
            r=random.random()
            idx = int(min(multi_start_count,len(solutions))*r*r)
            ...
            update_best_solution()
            update_region_pool_all()
            #print "R&R",ruin_idx, int(biobjective),int(objective),int(objective_overload),
            ...
            all_solutions=solutions
            t2=time.time()-t
            print acceptanceRule,loop,idx,"bestg",int(best_biobjective_global*100)/100.0,int(best_overload_global),
            print "current", int(biobjective*100)/100.0,int(objective_overload),move_count, int(t2*100)/100.0, not_improve_best
            if time.time()-t_h > heuristic_time_limit: break
    if acceptanceRule=="hc" or acceptanceRule=="vnd" or acceptanceRule=="sls" or acceptanceRule=="rrt":
        ...
        while 1:
            ...
            #repare_current_solution()
            feasible=check_solution_continuality_feasibility(node_groups)
            if feasible==0:print "check_solution_continuality_feasibility(node_groups)..."
            #update_district_info()
            update_best_solution()
            best_solution=node_groups[:]
            best_biobjective=biobjective
            ...
            if acceptanceRule=="vnd":
                ...
                #VND_local_search()
                #update_region_pool_all()
                print acceptanceRule,idx,"bestg",int(best_biobjective_global*100)/100.0,int(best_overload_global),
                #print "cbest", int(best_biobjective),int(best_objective),int(best_objective_overload),
                print "current", int(biobjective*100)/100.0,int(objective_overload),move_count
                if time.time()-t_h >heuristic_time_limit: break
                continue
            if acceptanceRule=="hc":
                ...
                while 1:
                    ...
                    RRT_local_search()
                    update_region_pool_all()
                    print acceptanceRule,idx,loop,"bestg",int(best_biobjective_global),int(best_objective_global),int(best_overload_global),
                    print "cbest", int(best_biobjective),int(best_objective),int(best_objective_overload),
                    print "current", int(biobjective),int(objective),int(objective_overload),move_count
            ...
            if acceptanceRule=="sls" or acceptanceRule=="rrt":
                ...
                while 1:
                    ...
                    print acceptanceRule,idx,loop,"bestg",int(best_biobjective_global),int(best_objective_global),int(best_overload_global),
                    print "cbest", int(best_biobjective),int(best_objective),int(best_objective_overload),
                    print "current", int(biobjective),int(objective),int(objective_overload),move_count,dev_move_counter,
                    if acceptanceRule=="sls": print int(sls_move_possibility*10000)
                    else: print devition
                    #if time.time()-sls_t >heuristic_time_limit/multi_start_count: break
        ...
        while 1:        
            ...
            for loop in range (maxloops):
                ...
                if random.random()>0.5:
                    ...
                    update_district_info()
                    #update_best_solution()
                SA_coolingrate=math.exp( math.log(0.005)/maxloops)
                SA_temperature=init_temperature*pow(SA_coolingrate,loop)
                last_loop_obj=biobjective
                RRT_local_search()
                update_region_pool_all()
                print acceptanceRule,idx,loop,"T=", int(SA_temperature*1000),"bestg",int(best_biobjective_global),int(best_objective_global),int(best_overload_global),
                print "cbest", int(best_biobjective),int(best_objective),int(best_objective_overload),
                print "current", int(biobjective),int(objective),int(objective_overload),move_count,int((time.time()-t_h)*1000)/1000.0
            if time.time()-t_h > heuristic_time_limit: break
    ...
    #print "bestg",int(best_biobjective_global),int(best_objective_global),int(best_overload_global),
    #print "bestc",int(best_biobjective),int(best_objective),int(best_objective_overload)
    
    print "global best solution:",best_biobjective_global,best_objective_global,best_overload_global,time.time()-t
    ...
    if is_spp_modeling==1 and acceptanceRule!="mip":
        if sppmodel()>=0:
            update_district_info()
            update_best_solution()
            print "spp solution:",best_biobjective_global,best_objective_global,best_overload_global,time.time()-t
    feasible=check_solution_continuality_feasibility(best_solution_global)
    if feasible==0:
    ...
    print "time_check,time_check_edge_niut,time_spp",time_check,time_check_edge_niut,time_spp
    print "move stat",count_op0,count_op1,count_op2,count_op3,count_op4
    print "check_count",check_count
    
#solve the problem
#must read instance, set districting parameters, and set solver parameters        
...
def spp_mst_file(mip_mst_fn):
    ...
    if len(feas_sol)<num_districts:
        print "len(sol)!=len(feas_sol) in spp mipstart"
...
def sppmodel():
    ...
    if len(region_pool)<=10:
        print "no candidate district!"
        return 0
    #print "preparing data...total regions:",len(region_pool)
    alpha_coeff=avg_dis_min*pop_dis_coeff  
    ...
    if mip_solver=='cplex':
        spp_mst_file("_mst.sol")
    else:
        mips_start("sol.txt")
    cbc=pulp.solvers.COIN_CMD(maxSeconds=100,mip=1,msg=0,fracGap=0.00001,options=['vnd on', 'node hybrid', 'rens on','mips sol.txt'])
    #if num_units>1000: cbc=pulp.solvers.COIN_CMD(maxSeconds=100,mip=1,msg=1,options=['vnd on', 'node hybrid', 'rens on'])
    #print "Solving spp model..."
    if mip_solver=='cplex':
        cbc=pulp.solvers.CPLEX_CMD(msg=0,timelimit=100,options=['set threads 6','set mip tolerances mipgap 0.0000001', 'read _mst.sol'])
    cbc.actualSolve(prob)
    if prob.status<0:
...
def best_mutation_rate(stats1,stats2):
    ...
    print "diff of new solutions:",[d1,d2],[d3,d4],cdiff,mdiff
...
def initial_solution(idx):
    ...
    method=idx%2
    if mip_solver not in mip_solvers: method=2
...
def ga(numf,m,timelimit,mthd,crate, mrate,spp,myseed):
    ...
    while 1: #multi_start<multi_start_count:
        ...
        t2=time.time()
        if multi_start==1 and (mip_solver=="cbc" or mip_solver=="cplex"):
            print str(multi_start)+"*",
        ...
        print "initial solution",biobjective,objective, objective_overload,int((time.time()-t2)*100)/100.0, int(opt*10000)/10000.0
        multi_start+=1
        if len(population)>= multi_start_count:
            break
    population.sort(key=lambda x:x[0])
    print "best init solution, and optimality",population[0][0],int((time.time()-t_ga)*100)/100.0,optimum
    stats=[]
    evolution_c_stats=[]
    evolution_m_stats=[]
    stats_mutation=[]
    print "lower_bound on obj:",lp_obj
    #t_ga=time.time()
    not_improved=0
    spp_loops=max(multi_start_count*2,40)
    loop=0
    while 1:
        loop+=1
        ...
        print [diff1,diff2],
        print [int(cur_crate*1000)/1000.0,int(cur_mrate*1000)/1000.0],

        sol1=population[idx1][2]
        sol2=population[idx2][2]
        diffunits=0
        optimum=(population[0][0]-lp_obj)/lp_obj
        p_obj=biobjective=population[idx1][0]
        #step2: crossover
        #cur_crate=0.5+0.4*random.random()
        if random.random()<0.3 or mthd==9:
            print 9,
            node_groups=sol1[:]
            update_district_info()
        else:
            cur_crate=1-mrate/diff1*100
        ...
        population.append([biobjective,centersID[:],node_groups[:]])
        print "gen",loop,idx1,idx2,int(diffunits),diffunits2,
        print "cur", int(biobjective*100)/100.0,int(objective_overload),"best",int(best_biobjective_global*100)/100.0, int((time.time()-t_ga)*100)/100.0,not_improved,

        #local optimum: renew the population, and reselection by spp modeling 
#        if is_spp_modeling==1 and loop%spp_loops==spp_loops-1:
        if (not_improved%spp_loops==spp_loops-1):
        ...
        if is_spp_modeling==1 and (not_improved%spp_loops==spp_loops-1):
            maxp=min(multi_start_count,len(population))-1
            if maxp%2==0: maxp-=1
            for i in range (maxp, 2,-2):
                del population[i]
            print "*",
        ...
        print [b1,b2,len(population)], not_improved

        #step5 adjust mutation rate
        cur_mrate=(1+random.random())*mrate/2
##        if adjust_mutation_rate==1:
##            if diff1<8.0:
##                #cur_mrate=(1+random.random())*mrate/2
##                #cur_mrate=0.005+random.random()*0.015
##                cur_mrate=0.005+random.random()*mrate/2
##                cur_mrate=mrate
##                #evolution_c_stats=[]
##                #evolution_m_stats=[]
##            elif len(evolution_c_stats)>=multi_start_count:
##                diff_sol_c,diff_sol_m,cd,md=best_mutation_rate(evolution_c_stats,evolution_m_stats)
##                avg_rate=sum(x[1] for x in evolution_m_stats)/len(evolution_m_stats)
##                if diff_sol_m < 10: cur_mrate=avg_rate+mrate/4 #=0.015+random.random()*0.015
##                elif diff_sol_m > 20: cur_mrate=avg_rate-mrate/4 #=0.005+random.random()*0.005
##                else: cur_mrate=sum(x[1] for x in evolution_m_stats)/len(evolution_m_stats)
##                #0.005+random.random()*mrate/2
##                if cur_mrate<0.005: cur_mrate=0.005
##                if cur_mrate>mrate: cur_mrate=mrate
##                evolution_c_stats=[]
##                evolution_m_stats=[]
##                print "mrate adjusted",diff_sol_c,diff_sol_m,cd, md, cur_mrate
##        else: cur_mrate=(1+random.random())*mrate/2
        #cur_mrate=(1+random.random())*mrate/2
    ...
    print "global best solution:",biobjective,objective,objective_overload,ga_time
    t_spp=time.time()
    if is_spp_modeling==1:
        sppmodel()
        update_district_info()
        print "number of areas recorded", len(region_pool)
        print "spp solution:",biobjective,objective,objective_overload,time.time()-t_spp, time.time()-t
    RRT_local_search()
    print "final LS solution:",biobjective,objective,objective_overload
    
    feasible=check_solution_continuality_feasibility(node_groups)
    if feasible==0:
        print "infeasible final solution: continuality "
    else:
        print "feasible final solution: continuality "
    ...
    print "time_check,time_check_edge_unit,time_spp",time_check,time_check_edge_niut,time_spp
    print "check_count",check_count
    print "not_improved loops",not_improved
    print "DistrictId, No.units, pop, cap, dis"
    for i in range(num_districts):
        print i,district_info[i][0],district_info[i][1],district_info[i][3], int(district_info[i][2]*100)/100
...
def instance_NNN_solution():
    ...
    for i in range(num_units):
        ...
        dis=9999999.99
        for k in range(num_districts):
            if nodedij[i][centersID[k]]<dis:
                dis=nodedij[i][centersID[k]]
                idx=k
        node_groups[i]=idx
    update_district_info()
...
def near_facilities(n):
    global NFlist
    NFlist=[]
    for i in range(num_units):
        fdlist=[[nodedij[i][centersID[k]],k] for k in range(len(centersID))]
        fdlist.sort(key=lambda x:x[0])
        flist.append(x[1] for x in fdlist)
        NFlist.append(flist)    
    return
...
##        obj += alpha_coeff*tvariables[x]
    ...
    if mip_solver=="cplex":
        cbc=pulp.solvers.CPLEX_CMD(options=['set mip tolerances mipgap 0.003'])
        if rand==0:
            cbc=pulp.solvers.CPLEX_CMD(options=['set mip tolerances mipgap 0.0000001'])
    cbc.actualSolve(prob)
...
##        obj += alpha_coeff*tvariables[x]
    ...
    for k in range(num_districts):
        ...
        prob+=s <= capacities[k]
    prob.writeLP("_tp2.lp")
    cbc=pulp.solvers.COIN_CMD(mip=1,msg=0,maxSeconds=10,fracGap = 0.00001)
    if mip_solver=="cplex":
        cbc=pulp.solvers.CPLEX_CMD(options=['set mip tolerances mipgap 0.000000001'])
    cbc.actualSolve(prob)
    ...
    for v in prob.variables():
        if (v.varValue >= 0.90):
            ...
            i=int(items[1])
            k=int(items[2])
            if v.varValue < nodes[i][3]:
                fragunit.append([i,k,v.varValue])
            if v.varValue>sol[i][1]:
                sol[i][0]=k
                sol[i][1]=v.varValue
    node_groups=[x[0] for x in sol]
    update_district_info()
    ulist = set ([x[0] for x in fragunit])
    #klist = set ([x[1] for x in fragunit])
    fullunit=0
    for k in range(num_districts):
        f=int(sum(x[1] for x in sol if x[0]==k))-capacities[k]
        if f>=0:fullunit+=1
    print len(ulist), len(fragunit), sum(nodes[x][3] for x in ulist),fullunit
    return 
